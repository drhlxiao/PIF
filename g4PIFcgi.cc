...
void Help() 
{
    ...
    G4cout<<"Options:"<<G4endl
        ...
        <<"-write_tree          write event tree          ."<<G4endl
...
int main(int argc,char **argv) 
{
    ...
    while(s<argc-1)
    {
        ...
        else if(sel=="-write_tree")
        {
			write_event_tree=true;
			++s;
		}
        else if(sel=="-td")
        {
   ...
   G4VModularPhysicsList* physlist = factory.GetReferencePhysList(plname);
    ...
    delete runManager;
    G4cout<<"Simulation output: "<<outputFilename<<G4endl;
